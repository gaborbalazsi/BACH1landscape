% important code: BL simulations for paper
% it is not only Gaussian, as the filename would sugest
% Produces main Fig. 4e,f, Ext. figure panels S9g,h 
...
%current Dox concentration
    ...
    Dmean1=a1*varNF/mean(a1*ExpSeeded+b1);    %1st expected change in mean (from theory)
    Dmean2=(2*a2*muNF+b2)*varNF/mean(a2*ExpSeeded.^2+b2*ExpSeeded+o2);   %1st expect. change in variance
%    Dmean2=(a2*cov_z_z2+b2*varNF)/mean(a2*ExpSeeded.^2+b2*ExpSeeded+o2);   %1st expect. change in variance

%    Dvar1=-a1^2*varNF^2/mean(a1*ExpSeeded+b1)^2;    %1st expected change in mean (from theory)

%either of the next 2 works:
    Dvar1=a1*(cov_z_z2-2*muNF*varNF)/mean(a1*ExpSeeded+b1)-(a1*varNF)^2/mean(a1*ExpSeeded+b1)^2;    %1st expected change in mean (from theory)
...
%this quad theory is iffy. Double check!
%    Dvar2=a2*(var(ExpSeeded.^2)+(b2/a2-2*muNF)*cov_z_z2-2*muNF*b2*varNF/a2)/mean(a2*ExpSeeded.^2+b2*ExpSeeded+o2)-(a2*cov_z_z2+b2^2*varNF^2)^2/mean(a2*ExpSeeded.^2+b2*ExpSeeded+o2)^2;   %1st expect. change in variance
%next one works:
    ...
    Nmean1(ctr)=muNF+Dmean1;  %new mean (from linear theory) for invaded cells
    Nvar1(ctr)=varNF+Dvar1;   %new variance (from linear theory)
   ...
   title(sprintf('Dox=%.2f',doxC));
    ...
    %Gaussian histograms
    %stdEst=muNF*quadfit(muNF);xEst=pdf('Normal',xCenters,muNF,stdEst);plot(xCenters,xEst*max(mean(xNFh))/max(xEst),'kx');
    ...
    %plot(xCentExp,sBLexp(currind,:)*max(mean(xNFh))/max(sBLexp(currind,:)),'k--');
    %plot(xCentExp,iBLexp(currind,:)*max(mean(yNFh))/max(iBLexp(currind,:)),'m--');
